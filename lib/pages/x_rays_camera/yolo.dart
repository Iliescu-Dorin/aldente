...
import 'package:aldente/pages/x_rays_camera/nms.dart';
...
class YoloModel {
  ...
  List<List<double>> infer(Image image) {
    ...
    // output shape:
    // 1 : batch size
    // 4 + numClasses: left, top, right, bottom and probabilities for each class
    // num predictions
    final outputShape = _interpreter!.getOutputTensor(0).shape;
    final numPredictions = outputShape[2];
    final output = [
      List<List<double>>.filled(
          4 + numClasses, List<double>.filled(numPredictions, 0))
    ];
